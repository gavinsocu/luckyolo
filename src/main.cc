...
int main(int argc, char *argv[]) {
  system("RkLunch-stop.sh");
    ...
    int ret;
	const char *model_path = "./model/people.rknn";
		...
		if(s32Ret == RK_SUCCESS)
		{
			...
			{			
				...
				{		
					...
					vector<int> rect = {sX,sY,eX,eY};
					rects.push_back(rect);
				}
			}

			vector<vector<int>> results = tracker.custom_update(rects);
			...
			for (size_t i = 0; i < results.size(); ++i) {
				...
				sprintf(text, "personId<%d> %.1f%%", personId, det_result->prop * 100);
				cv::putText(frame,text,cv::Point(sx, sy - 8),
										cv::FONT_HERSHEY_SIMPLEX,1,
										cv::Scalar(0,255,0),2);

				if(personId > new_personId){
					new_personId = personId;
					char filename[50];
					sprintf(filename, "%s/personId[%d]_%.1f%%.jpg", IMG_FILENAME, personId, det_result->prop * 100);
					cv::imwrite(filename,frame);
				}

			}
			sprintf(fps_text,"fps = %.2f",fps);	
			cv::putText(frame, fps_text, cv::Point(40, 40),  // 左上角位置
